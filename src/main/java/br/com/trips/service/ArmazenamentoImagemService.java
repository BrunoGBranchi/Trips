...
public class ArmazenamentoImagemService {
	...
	public Imagens armazenaArquivo(MultipartFile arquivo) throws Exception {
		...
		try {
			...
			Imagens imagem = new Imagens(nomeArquivo, arquivo.getContentType(), arquivo.getBytes());
			
			return imagensRepository.saveAndFlush(imagem);
			
		} catch (IOException ex) {
			throw new FileStorageException("Não pode armazenar o arquivo " +nomeArquivo+", por favor tente novamente!");
		}
		
	}
	
	public Imagens getImagem(Long id) {
		return imagensRepository.findById(id).orElseThrow(() -> new ArquivoNaoEncontradoException("Arquivo não encontrado"));
	}
